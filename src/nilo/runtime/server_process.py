# File: src/nilo/runtime/server_process.py
# Format: UTF-8
# =============================
# File Description:
# Runtime-owned background process lifecycle for the local MCP server.
# TAG: runtime, mcp, server, process, lifecycle
# =============================
...
from nilo.core.config_constants import STDIO_TRANSPORT, STREAMABLE_HTTP_TRANSPORT
...
DEFAULT_PORT = 8000
DEFAULT_TRANSPORT = STREAMABLE_HTTP_TRANSPORT
...
# --------------------------------
# Function Description:
# Runs the MCP runner as a foreground stdio child process with inherited streams.
# Inputs/Outputs:
# No input; returns None on exit code zero or raises ServerProcessError.
# Usage:
# run_foreground_stdio_server()
# --------------------------------
def run_foreground_stdio_server() -> None:
    command = build_server_command(DEFAULT_HOST, DEFAULT_PORT, STDIO_TRANSPORT)
    try:
        completed = subprocess.run(command, check=False)
    except OSError as exc:
        raise ServerProcessError(f"Could not start foreground stdio server: {exc}") from exc
    if completed.returncode != 0:
        raise ServerProcessError(
            f"Foreground stdio server exited with code {completed.returncode}"
        )
